...
class BezierRopeView: UIView {
    ...
    private func setupInitialPoints() {
        // Place endpoints left/right center
        let w = bounds.width
        let h = bounds.height
        ...
        // Initial control points mid-way with small offsets
    ...
    @objc private func step(_ link: CADisplayLink) {
        ...
        // Slightly different target strengths for more interesting behavior
        ...
        // If user is dragging, override the target for the dragged point
        var finalTarget1 = target1
        var finalTarget2 = target2
        if let dragging = draggingPoint {
            // nothing: draggingPoint's pos is moved by touch handlers
        }

        // Integrate physics for p1
        do {
            let pos = p1.pos
            ...
            if draggingPoint != nil && draggingPoint == withUnsafeMutablePointer(to: &p1, { $0 }) {
                // if dragging p1, skip physics so drag feels direct
            } else {
                p1.apply(acc: CGPoint(x: ax, y: ay), dt: dt)
            }
        }

        // Integrate physics for p2
        do {
            let pos = p2.pos
    ...
    // MARK: - Input handling (touch)
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard let t = touches.first else { return }
        let loc = t.location(in: self)
        // choose nearest dynamic control if within radius
        let r: CGFloat = 30
    ...
    override func draw(_ rect: CGRect) {
        ...
        // background subtle grid or gradient (optional)
        ...
        // draw control point lines for clarity (optional)
        ctx.setLineWidth(1)
